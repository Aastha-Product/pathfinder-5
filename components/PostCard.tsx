...
export const PostCard: React.FC<PostCardProps> = ({ post: initialPost, onDm, onPostDeleted }) => {
  ...
  const isMyPost = post.author.name === 'You'; // Mock auth check
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm
            ${post.author.name === 'You' ? 'bg-primary-600' : post.author.name === 'Sarah Chen' ? 'bg-indigo-500' : 'bg-blue-500'}`}
          >
            {post.author.avatar}
          </div>
          <div>
            <div className="flex items-center flex-wrap gap-2">
              <h3 className="font-bold text-[#0f172a]">{post.author.name}</h3>
              {post.author.level && (
                                  ...
                                  <div key={comment.id} className="group relative">
                                      <div className="bg-slate-50 p-3 rounded-r-xl rounded-bl-xl text-sm border border-slate-100">
                                          <div className="flex justify-between items-center mb-1">
                                              <span className="font-bold text-[#0f172a] text-xs">{comment.author.name}</span>
                                              
                                              {/* Actions & Timestamp grouped to prevent overlap */}
                                              <div className="flex items-center gap-2">
                                                  {/* Comment Actions (Edit/Delete) - only for own comments */}
                                                  {comment.author.name === 'You' && !editingCommentId && (
