...
export const Projects: React.FC<ProjectsProps> = ({ onNavigate }) => {
  ...
  const [projects, setProjects] = useState(MOCK_PROJECTS_LIST);
  const [currentProject, setCurrentProject] = useState(MOCK_PROJECTS_LIST[0]);
  ...
  const handleProjectSave = (data: any) => {
      if (projectToEdit) {
          ...
          // Create new
          const newProj = { id: Date.now().toString(), ...data };
  ...
  const handleProjectDelete = () => {
      if (projectToDelete) {
          ...
          if (currentProject.id === projectToDelete && newProjects.length > 0) {
  ...
  const handleInvite = (emails: string[]) => {
    setToastMessage(`Invites sent to ${emails.length} people via magic link`);
    setShowToast(true);
    setTimeout(() => setShowToast(false), 3000);
  };
  ...
  return (
    <div className="h-screen bg-white flex flex-col overflow-hidden">
        ...
        {/* --- Main Content Area (Scrollable containers inside) --- */}
                                         ...
                                         return (
                                             <div key={task.id} className="flex items-center group cursor-pointer" onClick={() => { setEditingTask(task); setIsTaskModalOpen(true); }}>
                                                 ...
                                                 <div className="w-3/4 relative h-8 bg-slate-50 rounded-lg overflow-hidden border border-slate-100">
                                                     <div 
                                                         className={`absolute top-1 bottom-1 rounded-md opacity-90 transition-all group-hover:opacity-100 ${
